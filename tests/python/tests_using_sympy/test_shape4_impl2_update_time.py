"""
Test spectra_wave_shape_4 impl=2 for update_time
"""
    ...
    def checks(ts, analytic=True):
        ...
        for t in ts:
            ...
            if analytic:
                ...
                assert math.isclose(phi_num, phi_anal, rel_tol=1e-02, abs_tol=1e-02)
            res_phi.append(phi_num)

            elev_num = swd_num.elev(x, y)
            if analytic:
                elev_anal = swd_anal.elev(x, y, t)
                assert math.isclose(elev_num, elev_anal, rel_tol=1e-02, abs_tol=1e-02)
