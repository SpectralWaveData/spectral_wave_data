module spectral_wave_data_shape_5_impl_1_def
...
! This module provides an implemention of the shape 5 class of the 
...
!------------------------------------------------------------------------------
!
public :: spectral_wave_data_shape_5_impl_1
!
!------------------------------------------------------------------------------
!
...
type, extends(spectral_wave_data) :: spectral_wave_data_shape_5_impl_1
...
class(spectral_wave_data_shape_5_impl_1) :: self  ! Object to destruct
!
logical opened
!
call self % fft % close()
...
self % fft = swd_fft(self % nsumx, self % nsumy, self % dkx, self % dky, self % d)
...
complex(c_float), parameter :: czero_c = cmplx(0.0_c_float, 0.0_c_float, c_float)
character(len=*), parameter :: err_proc = 'spectral_wave_data_shape_5_impl_1::update_time'
...
class(spectral_wave_data_shape_5_impl_1), intent(in) :: self  ! Actual class
...
do jx = 0, self % nsumx
    ...
    do jy = 0, self % nsumy
        ...
        phi_xswd = phi_xswd + kjx * Cfun1 % im * Zfun
        phi_yswd = phi_yswd + kjy * Cfun2 % im * Zfun
        phi_zswd = phi_zswd + Cfun1 % re * Zfun_z
    end do
end do
...
do jx = 0, self % nsumx
    ...
    do jy = 0, self % nsumy
        ...
        Cfun1 = (CfunA + CfunB) * Xfun
        Cfun2 = (CfunA - CfunB) * Xfun  ! d(conjg(Yfun))/dy = - d(Yfun)/dy
        phi_xtswd = phi_xtswd + kjx * Cfun1 % im * Zfun
        phi_ytswd = phi_ytswd + kjy * Cfun2 % im * Zfun
        phi_ztswd = phi_ztswd + Cfun1 % re * Zfun_z
    end do
end do
...
real(c_float) dkx, dky, grav, lscale, dt, d, magic
character(kind=c_char, len=:), allocatable :: cid
character(kind=c_char, len=30) :: cprog
character(kind=c_char, len=20) :: cdate
complex(c_float), allocatable :: camp(:)
character(len=*), parameter :: err_proc = 'spectral_wave_data_shape_5_impl_1::strip'
character(len=250) :: err_msg(5)
!
if (tmin >= tmax) then
    write(err_msg(1),'(a,a)') 'SWD file: ', trim(self % file)
...
!
character(len=*), parameter :: err_proc = 'spectral_wave_data_shape_5_impl_1::get_int'
...
!
character(len=*), parameter :: err_proc = 'spectral_wave_data_shape_5_impl_1::get_logical'
character(len=250) :: err_msg(1)
!
select case(name)
case('dc_bias')
...
!
character(len=*), parameter :: err_proc = 'spectral_wave_data_shape_5_impl_1::get_real'
character(len=250) :: err_msg(1)
!
select case(name)
...
!
character(len=*), parameter :: err_proc = 'spectral_wave_data_shape_5_impl_1::get_chr'
character(len=250) :: err_msg(1)
!
select case(name)
case('file', 'file_swd')
...
case('version')
    res = version
case('class')
    res = 'spectral_wave_data_shape_5_impl_1'
case('prog')
    res = self % prog
case('date')
    res = self % date
...
complex(wp) :: c_fft(self % nsumx + 1, 2*self % nsumy+ 1)
real(wp), dimension(self % nsumx + 1, 2*self % nsumy + 1) :: Zfun, Zhfun, kz, coshkz, sinhkz
character(len=*), parameter :: err_proc = 'spectral_wave_data_shape_5_impl_1::grad_phi_fft'
character(len=:), allocatable :: err_msg(:)
...
c_fft = self % fft % swd_to_fft_coeffs_2D(self % c_cur(:, 0:self % nsumy, 0:self % nsumx))
phi_x = self % fft % fft_field_2D(iu*self % fft % kx*c_fft*Zfun, nx_fft_in, ny_fft_in)
allocate(grad_phi(3, size(phi_x,1), size(phi_x,2)))
grad_phi(2, :, :) = self % fft % fft_field_2D(iu*self % fft % ky*c_fft*Zfun, nx_fft_in, ny_fft_in)
grad_phi(3, :, :) = self % fft % fft_field_2D(self % fft % k*c_fft*Zhfun, nx_fft_in, ny_fft_in)
...
end module spectral_wave_data_shape_5_impl_1_def
